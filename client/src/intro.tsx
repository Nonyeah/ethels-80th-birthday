
export default function Intro() {
  return (
    <div className="intro-container">
      <h4>Save the date: November 1st 2025</h4>

      <h1>Ethel is turning 80!</h1>

      <p>To Christ be the glory!</p>
      <p>
        Come help us celebrate the 80th birthday of our beloved mother,
        grandmother, sister and friend to many, Mrs Ethel who on 31st October
        2025 will have been alive to witness the earth rotate around the sun 80
        times, wow! We are truly grateful to God for blessing our mother with
        such a long and fruitful life and we pray our Good God will continue to
        satisfy her with long life and good health
      </p>
      <p>
        Come join us as we celebrate! Let's eat some delicious food, drink until
        we are merry and <p><span>DANCE DANCE DANCE!</span></p>
      </p>
    </div>
  );
}
